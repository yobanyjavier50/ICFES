...
class RealQuizScreen extends StatefulWidget {
  ...
  final bool esReal;
  const RealQuizScreen({super.key, required this.preguntas, required this.esReal});
  @override
...
class _RealQuizScreenState extends State<RealQuizScreen> {
  ...
  void initState() {
    super.initState();
    if (widget.esReal) _timer = Timer.periodic(const Duration(seconds: 1), (t) => setState(() => _t > 0 ? _t-- : _finish()));
  }
  ...
  void _finish() async {
    ...
    if (porcentaje >= 0.6) {
      AudioService.playSuccess(); // Pasó la prueba
    } else {
      AudioService.playFail(); // Necesita mejorar
    }
    ...
    await GoogleSheetsService.guardarResultado(prefs.getString('email')??"", prefs.getString('grupo')??"", widget.esReal?"Real":"Practica", score);
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text("Feedback", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: azulProfundo)),
                IconButton(
                  icon: CircleAvatar(
            ...
            Text(
              (retro.trim().isEmpty) ? "Sin retroalimentación para esta pregunta." : retro,
  ...
  Widget build(BuildContext context) {
    if (widget.preguntas.isEmpty) return const Scaffold(body: Center(child: Text("No hay preguntas")));
    final p = widget.preguntas[_idx];

    return Scaffold(
      backgroundColor: Colors.white,
      appBar: AppBar(
        title: Text(widget.esReal ? "Tiempo: ${_t~/60}:${(_t%60).toString().padLeft(2,'0')}" : "Practice Mode", style: const TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Colors.white,
      ),
      body: Column(
        children: [
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  if (p['textoBase'] != null && p['textoBase'].toString().trim().isNotEmpty)
                    Container(
                  ...
                  if (p['img'] != null && p['img'].isNotEmpty)
                    Center(
                        child: ClipRRect(
                            borderRadius: BorderRadius.circular(15),
                            child: Image.network(p['img'], height: 200, errorBuilder: (_,__,___) => const Icon(Icons.broken_image))
                        )
                    ),

                  const SizedBox(height: 20),
                  Text(p['pregunta'], style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: azulProfundo)),
                  const SizedBox(height: 20),
                  ...
                  ...p['opciones'].map<Widget>((o) {
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        color: isSelected ? azulProfundo.withValues(alpha: 0.05) : grisCampos,
                        borderRadius: BorderRadius.circular(20),
                        border: Border.all(color: isSelected ? azulProfundo : Colors.transparent, width: 2),
                      ),
                      child: RadioListTile<String>(
                          title: Text(stringValue, style: TextStyle(fontWeight: FontWeight.bold, color: azulProfundo)),
                          value: stringValue,
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    if (_idx < widget.preguntas.length - 1) setState(() => _idx++); else _finish();
                  },
                  child: Text(_idx < widget.preguntas.length - 1 ? "NEXT QUESTION" : "FINISH", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 1))
              ),
            ),
          )
